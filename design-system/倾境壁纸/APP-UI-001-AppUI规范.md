...
版本：0.1；日期：2026-09-14；任务：WP-UI01；当前状态：已实施，构建与红米页面核对通过，待用户最终视觉验收。进度以 [PM-002 第 14 节](../../docs/10-项目管理/PM-002-数据库API与H5联调开发计划.md#14-app-样式与内容调整) 为准。
...
本步骤提供共用主题、基础组件与对照页。首页、我的、详情等业务页的整体内容和结构同步属于 WP-UI02，尚未开始；不能据此宣称全站已经与 H5 一致。
...
| 品牌栏 / 微信客服 / 设置教程播放器 | WP-UI02 | 业务页内容和布局后续同步，本步骤不重做业务流程 |
...
WP-UI02 按 H5 页面逐一同步，未经用户要求不自行增加内容：
...
本轮核对与构建记录见 [WP-UI01](../../docs/06-测试与验收/WP-UI01-AppUI规范验收-2026-09-14.md)。最终视觉和真实体验由用户验收。
