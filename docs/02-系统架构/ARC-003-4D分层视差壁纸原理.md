...
v2 使用一个全局满幅角度和每层方向/位移比例：

- `motion.maxAngle`：达到最大位移所需的相对倾斜角，范围 `1°～75°`；从 0° 开始响应，没有启动死区。
- `layer.offsetPercent`：满幅角度下相对屏幕轴长的位移百分比；非负且不设置产品上限。
- `layer.direction`：`follow` 跟随、`reverse` 反向、`fixed` 固定。
- 屏幕对应轴长为 `100%`；数值 100 移动一整屏，200 移动两屏，10 移动 `0.1` 屏。
...
左右和上下使用同一层位移比例。传感器噪声使用 App 内部固定平滑处理，不进入素材配置。v2 不使用 `depth`、`strength`、`responseCurve` 或 `responseGain`。
...
满幅时实际移动分别为屏幕轴长的 8% 和 3%。未达到 `maxAngle` 时按角度线性缩放；0° 位移为 0。这只是初始值，编辑器允许输入 100%、200%、300% 等更大的构图位移。
...
```text
实际满幅位移比例 = offsetPercent / 100
背景最低显示倍数 = 1 + 2 × 实际满幅位移比例
```

例如背景为 `3% / reverse`，实际满幅位移为屏幕轴长 3%，最低显示倍数为 `1.06`。背景比例更大时按同一公式继续增加显示范围；前景可以完全移出屏幕。超出屏幕的部分只在绘制时裁掉；源图片文件和解码分辨率不变。配置中的 `scale` 可以提供更大的艺术构图放大。
...
```mermaid
flowchart LR
    A[Web 模拟器调整每层方向和位移比例] --> B[导出 config.json 与图层 ZIP]
    B --> C[管理后台上传完整 ZIP]
...
Web 模拟器和 Android 共同拥有算法契约。API 不判断方向、位移比例关系或艺术效果，不把配置转换成另一套内部字段。正式签名包中的 `PARALLAX_CONFIG` 与上传 ZIP 中的 `config.json` 保持相同字节。
...
1. 满幅 75°、位移比例 100% 时移动一整屏，200% 时移动两屏；37.5° 时产生一半配置位移，0° 时为 0。
2. 同一位移比例切换 follow/reverse 时方向立即反转；fixed 或 0% 不移动。
3. 同一 v2 配置在 Web 和 Android 上的图层顺序、方向和满幅位移一致。
4. 背景达到最大位移时不露边；显示放大不改变源图字节和解码分辨率。
5. API 透传测试可以证明未知算法字段字节不变；正式发布只允许当前 Android 已支持的 `formatVersion` 和字段集合。
