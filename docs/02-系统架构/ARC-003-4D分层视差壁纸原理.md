...
完整 Schema 见 [`parallax-source-v2.schema.json`](../../packages/wallpaper-format/parallax-source-v2.schema.json)。当前产品尚未正式上线，Web 导出、API 交付和 Android 运行时统一只使用 v2，不保留 v1 `depth/strength` 分支。
...
6. v1 源包和旧 Android 内部配置均被拒绝，链路中不再产生 `depth/strength/responseCurve/responseGain`。
