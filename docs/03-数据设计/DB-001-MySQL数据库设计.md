...
| 表/字段 | 约束与用途 |
|---|---|
| `parallax_source_package` | 原 ZIP 私有存储键、SHA-256 唯一、文件名/大小、画布、创建人；封面及配置引用 Asset；V6 后配置为原始 v2 字节 |
| `parallax_source_layer` | 源包与图层编号联合主键；源包/role/ordinal 唯一；V6 后只保存文件结构和资产关系 |
...
Flyway `V6__parallax_v2_config_passthrough.sql` 清理尚未上线的 v1 测试源包、关联正式/预览包及旧资源绑定，并把可安全删除的运行时对象加入清理队列。V5 保持字节不变。

- `parallax_source_package.format_version` 固定为 2，`config_asset_id` 指向上传 ZIP 中的原始 `config.json`。
