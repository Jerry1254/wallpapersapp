...
**日期：** 2026-09-17
**对应任务：** WP-UI13
**状态：** Java API、OpenAPI 2.1.0 与真实 MySQL/Redis 集成验证已通过，App 待按本文确认接入
...
- [ ] 不再进行分页后的能力过滤。
- [ ] 多能力商品在每个列表中只显示一张卡片。
- [ ] 多能力商品响应保留完整 `availableCapabilities`。
- [ ] 能力筛选可以与分类、免费、精选和搜索组合。
- [ ] 空结果按 `totalItems=0`、`totalPages=0` 正常结束加载更多。
- [ ] 缓存按能力参数和 `profileHash` 隔离。
