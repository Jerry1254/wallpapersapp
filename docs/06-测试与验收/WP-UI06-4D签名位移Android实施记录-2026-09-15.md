...
**结论：** Android 只接受 v2 配置，签名位移解析和绘制已完成，v1 `depth/strength` 路径已删除。Web 模拟器、API 透传和管理后台改造已输出独立实施方案，本记录不把尚未实施的端到端 v2 链路写成已联调。
...
结果：`BUILD SUCCESSFUL`，27 项通过，0 失败、0 错误、0 跳过。覆盖 v2 前后图层顺序、正负位移反向、0 固定语义、背景最大位移不露边、75° 满幅、v1 拒绝、配置边界和签名包安装。
