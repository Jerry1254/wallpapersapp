# WP-UI06 4D 方向与位移比例 Android 实施记录

**日期：** 2026-09-15

**结论：** Android 只接受当前 v2 配置，方向与开放位移比例解析和绘制已完成，旧带符号位移及 v1 `depth/strength` 路径不再接受。API 透传和管理后台改造仍是独立任务。

## 已实施

- Android 仅识别 `formatVersion=2` 配置，源图层从 `01` 最前景到最后背景，运行时自动反转为从背景向前景绘制。
- 每层使用 `offsetPercent` 位移比例和独立 `direction`。`offsetPercent` 为非负有限数值，不设置产品上限；100% 在满幅角度移动一整屏，200% 移动两屏。`follow` 跟随、`reverse` 反向、`fixed` 固定。
...
结果：`BUILD SUCCESSFUL`，27 项通过，0 失败、0 错误、0 跳过。覆盖 v2 前后图层顺序、方向切换、0%/固定语义、100% 一整屏、300% 移出屏幕、背景满幅不露边、75° 满幅、旧配置拒绝和签名包安装。
