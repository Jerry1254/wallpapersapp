...
| `ONLINE_TEST` | 线上测试环境 | 尚未建设 | 独立 HTTPS 域名，待定 | 独立实例和 schema，待定 | 独立实例，待定 | 独立测试存储，待定 | 独立测试后台，待定 | 建议新增 `staging` flavor 与 `.staging` applicationId |
| `PRODUCTION` | 正式生产环境 | 尚未发布 | 独立 HTTPS 域名，发布前确定 | 生产专用实例和 schema | 生产专用实例 | 生产专用对象存储/CDN | 生产专用后台 | `prod` / `com.qingjing.bizhi` |
...
| `ONLINE_TEST` | 脱敏测试数据、待发布版本的验收数据 | 按测试周期保留 | 否 |
| `PRODUCTION` | 已审核并正式发布的业务数据 | 长期保留并备份 | 是 |

正式内容可以先在 `LOCAL_DEV` 制作和核对，但上线时必须通过受控的内容导入或正式 API 发布流程进入 `PRODUCTION`。不得把本地数据库、Redis 数据或整个本地资源目录直接复制到生产。
...
- `local`、`internal` 或 `lab` App 连接生产 API。
- 正式 `prod` App 连接本地或线上测试 API。
...
- `prod` 只允许正式生产地址。生产地址没有确定前，默认保留 `https://api.invalid/api/v1`，禁止为了方便把 `prod` 指向测试地址。
- `ONLINE_TEST` 建设时新增独立 `staging` flavor、独立 applicationId 和测试签名，不复用 `prod` 身份。完成之前不得把 `internal`、`lab` 或 `prod` 临时改成线上测试包。
...
- `ONLINE_TEST` 和 `PRODUCTION` 使用各自部署的后台地址。正式生产操作不得通过临时修改本地 Vite 代理完成。
...
`LOCAL_DEV` 和 `LOCAL_DEVICE` 可以在明确授权范围内清理。`ONLINE_TEST` 先备份或确认测试周期可重置。`PRODUCTION` 的删除、迁移、发布和数据修复必须走单独生产流程并取得明确确认；本地脚本不得接受生产地址或生产凭据。
...
`ONLINE_TEST` 建成后承担可公开访问的候选版本联调，不承担正式用户流量：

- 使用独立域名、数据库、Redis、存储、密钥、管理员账号和 App 签名。
- 只使用脱敏数据和测试兑换码，不导入生产账号、设备身份或生产数据库快照。
- App 使用独立 `staging` flavor，界面应持续显示“测试环境”，并可与正式 App 同机安装。
- 每次部署记录代码提交、OpenAPI 版本、Flyway 版本、App 版本和回滚点。
- 验收通过只代表候选版可进入发布审批，不代表生产已发布。

在 `ONLINE_TEST` 尚未建成前，不得把 `LOCAL_DEVICE` 描述为“线上测试”，也不得用生产身份临时承担线上测试。
...
- 发现问题先在 `ONLINE_TEST` 或可复现的本地环境验证修复；确需生产处理时只执行已批准的最小变更。
...
1. API 增加固定 `QJ_ENVIRONMENT_ID`，在 `/actuator/info` 和 `X-QJ-Environment` 响应头返回环境 ID。
2. 管理后台启动时读取 API 环境 ID，在页面固定显示环境标识；环境未知时禁止写操作。
3. App 构建写入期望环境 ID，首次请求发现 API 环境不匹配时拒绝继续，并显示实际值与期望值。
4. 数据库保存单例环境身份和随机 UUID；API 启动时同时核对配置中的环境 ID/UUID，防止 API 连到错误 schema。
5. 数据清理脚本提供 `plan` 和 `apply` 两步命令，输出目标摘要；生产环境在脚本层永久拒绝。
6. CI 校验 `prod` 只能使用生产 HTTPS 地址，`staging`、`local`、`internal`、`lab` 不能使用生产地址。
...
1. 当前 `local` 只连 `LOCAL_DEV`；`internal` 和 `lab` 只连 `LOCAL_DEVICE`；`prod` 只连 `PRODUCTION`。
2. 后续线上联调新增 `staging` flavor，不临时复用现有 flavor；建议 applicationId 为 `com.qingjing.bizhi.staging`。
3. App 内不提供用户可编辑的 API 地址。构建时写入目标环境 ID 和 API 地址，运行时核对 API 返回的环境 ID。
4. 每个测试包的名称或页面必须能辨认环境，交付文件名必须带 flavor、版本和环境 ID。
5. App 只通过 API 获取目录、权益和资源，永远不连接 MySQL、Redis 或资源目录。

## 13. 待确认项

建议按以下结论确认后实施：

- 建设 `ONLINE_TEST` 时新增独立 `staging` flavor 与 `.staging` applicationId。
- 本轮先实现 API、管理后台和 App 的环境 ID 握手，再实现数据库 UUID 与清理脚本两步门禁。
- 正式内容从 `LOCAL_DEV` 到线上使用受控导入/发布流程，不复制数据库和运行时目录。
