...
Flutter 页面通过本包的类型化类调用插件，不直接使用 MethodChannel。当前提供安装 RSA 签名身份、独立 Keystore 解密公钥、format 2 下载与安全安装，以及静态、视频与 4D 分层预览和系统设置入口。
...
clearUnused() 拒绝与下载同时运行，保留 active-home/active-lock、视频和 4D 服务各自的提交版本、系统设置待确认版本及所有预览/播放租约。单进程共用原子资源库，创建预览或服务不会重新清理下载暂存目录；原子版本更新保留旧资源直到显式清理。
...
4D 的 ParallaxWallpaperService 和 App 预览各有独立渲染线程、姿态传感器、图片与资源租约。旋转向量、首帧校准、分层覆盖和深度移动源自仓库 PoC；正式实现读取签名配置，支持 normal/screen/add 混合、透明度、四种屏幕方向及安全位移边界。总解码预算最多 48 MiB 并保留至少 16 MiB 可用堆余量；显示频率不超过 30 fps。不可见或 Surface 销毁后注销传感器并释放图片/租约，重新可见时从私有提交指针复验恢复；没有可用传感器或强度为零时仅绘制静态画面。视频与 4D 提交指针分开保存，兼容旧版视频指针，切换服务不会丢失另一服务的内容。性能和真机倾斜结论见 [WP-A08 记录](../../docs/06-测试与验收/WP-A08-4D原生迁移记录-2026-09-14.md)。
