...
internal class AtomicPackageStore(val root: File) {
    ...
    fun staging(): File = File(root, ".stage-${UUID.randomUUID()}").also { check(it.mkdir()) }
    ...
    fun current(slot: String): String? {
        requireSlot(slot)
    ...
    fun commit(staging: File, e: PackageExpectation, beforePointer: () -> Unit = {}): String {
