...
internal data class ParallaxLayer(val role: String,val ordinal: Int,val depth: Float,val scale: Float,val opacity: Float,val blend: String)
internal data class ParallaxConfiguration(val width: Int,val height: Int,val maxAngle: Float,val smoothing: Float,val strength: Float,val layers: List<ParallaxLayer>) {
    companion object {
        fun parse(bytes: ByteArray): ParallaxConfiguration {
            ...
            val layers = list.map { value ->
                ...
                ParallaxLayer(role,ordinal,next,number(layer["scale"],1.0,1.5),number(layer["opacity"],0.0,1.0),blend)
            }
