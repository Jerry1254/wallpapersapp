...
public class ParallaxPackageReader {
    ...
    public ParallaxPackageDtos.SourcePackage get(long id) {
        var rows=jdbc.query("""
                SELECT id,original_filename,size_bytes,sha256,cover_asset_id,canvas_width,canvas_height
                ...
                rs.getString("sha256"),rs.getLong("cover_asset_id"),rs.getInt("canvas_width"),rs.getInt("canvas_height")),id);
        if(rows.isEmpty()) throw new ApiException(HttpStatus.NOT_FOUND,"RESOURCE_NOT_FOUND","4D 源包不存在或不可用");
        var row=rows.get(0);
        var layers=jdbc.query("""
                SELECT layer_index,original_filename,role,ordinal,depth,scale,opacity,blend_mode
                FROM parallax_source_layer WHERE source_package_id=? ORDER BY layer_index
                """,(rs,n)->new ParallaxPackageDtos.Layer(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),
                rs.getDouble(5),rs.getDouble(6),rs.getDouble(7),rs.getString(8)),id);
        return new ParallaxPackageDtos.SourcePackage(Long.toString(id),row.name(),"application/zip",row.size(),row.sha(),
                "READY",assets.get(row.cover()),new ParallaxPackageDtos.Canvas(row.width(),row.height()),layers);
    }
    ...
    private record Row(long id,String name,long size,String sha,long cover,int width,int height) {}
}
